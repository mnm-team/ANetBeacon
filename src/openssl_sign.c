...
int verifylanbeacon(const unsigned char* msg, size_t mlen, struct open_ssl_keys *lanbeacon_keys)
{
	...
	if(rc & NO_PUBLIC_KEY == NO_PUBLIC_KEY)
		printf("Could not read public key at specified path %s.\n", lanbeacon_keys->path_To_Verifying_Key);
		return(rc);
...
//	rc = verify_it(msg, mlen - 256, &msg[mlen - 256], 256, vkey);
	...
	else printf("Verified signature on receiver side\n");
...
//	rc = sign_it(msg, mlen, sig, slen, skey);
	...
	*slen = req;
	...
	if(rc != 1) {
		printf("EVP_DigestSignFinal failed, return code %d, error 0x%lx\n, mismatched signature sizes %ld, %ld", rc, ERR_get_error(), req, *slen);
		return(ERR_get_error());
	}
	else printf("Created signature\n");
